...
let protocol = config.www.protocol;
...
if(protocol=='https') {
  let options = {
    key: fs.readFileSync(config.www.keyfile),
    cert: fs.readFileSync(config.www.certfile)
  };
  log.info('Read keyfile  from',config.www.keyfile);
  log.info('Read certfile from',config.www.certfile);
  var server = https.createServer(options, app);
}
else {
  var server = http.createServer(app);
  protocol = 'http';			// Default value, in case left blank etc
}
...
server.on('listening', () => {
  ...
  log.info('Express', 'WWW %s server listening on %s', protocol, bind);
